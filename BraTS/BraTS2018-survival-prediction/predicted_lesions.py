...
from utils import (
    Brats2018ValidationN4ITKFilePaths,
    AllSubjectID,
    FindOneElement,
    ReadImage,
)
from utils import (
    PredictedLesionMaskPath,
    Brats2018PredictedLesionsPaths,
    Brats2018PredictedLesionsProbMapMNI152Paths,
)
import paths
...
def Lesions2MNI152(necrosisInVol, edemaInVol, enhancingTumorInVol):
    ...
    assert (
        SubjectID(necrosisInVol)
        == SubjectID(edemaInVol)
        == SubjectID(enhancingTumorInVol)
    )
    ...
    subprocess.call(
        [
            ...
            refVol,
            "-out",
            necrosisInVol[:-7] + new_name_append,
            "-init",
            omat,
            "-applyxfm",
        ]
    ...
    subprocess.call(
        [
            ...
            refVol,
            "-out",
            enhancingTumorInVol[:-7] + new_name_append,
            "-init",
            omat,
            "-applyxfm",
        ]
...
parser.add_argument(
    "-m", "--mode", help="can be train, valid or test", default="train", type=str
)
parser.add_argument(
...
assert (
    len(predicted_lesions_paths) == 191
    or len(predicted_lesions_paths) == 285
    or len(predicted_lesions_paths) == 66
)
...
necrosis_paths, edema_paths, enhancing_tumor_paths = Brats2018PredictedLesionsPaths(
...
necrosis_mni_paths, edema_mni_paths, enhancing_tumor_paths = Brats2018PredictedLesionsProbMapMNI152Paths(
    predicted_path=predicted_path
)
all_ids = AllSubjectID(brats_path)
assert (
    len(all_ids)
    == len(necrosis_mni_paths)
    == len(edema_mni_paths)
    == len(enhancing_tumor_paths)
), "brats_path and mode mismatch!!!"
...
pool.map(
    ...
    zip(necrosis_mni_paths, edema_mni_paths, enhancing_tumor_paths, all_ids),
)
