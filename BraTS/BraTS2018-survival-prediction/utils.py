...
def Brats2018TestingN4ITKFilePaths(brats_testing_path=paths.brats2018_testing_dir):
    ...
    t1_filepaths = [
        ...
        for root, dirs, files in os.walk(brats_testing_path)
        for name in files
        if "t1" in name
        and "ce" not in name
        and "corrected" in name
...
def Brats2018ValidationN4ITKFilePaths(
    brats_validation_path=paths.brats2018_validation_dir
):
    """ This fucntion gives the filepathes of all MR images with N4ITK and ground truth"""
    t1_filepaths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(brats_validation_path)
        for name in files
        if "t1" in name
        and "ce" not in name
        and "corrected" in name
...
def PredictedLesionMaskPath(predicted_path):
    predicted_lesions_paths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(predicted_path)
        for name in files
        if "necrosis" not in name
        and "edema" not in name
        and "enhancing" not in name
        and "MNI152" not in name
        and name.endswith(".nii.gz")
    ]
...
def Brats2018PredictedLesionsPaths(predicted_path):
    ...
    necrosis_paths = [
        ...
        for root, dirs, files in os.walk(predicted_path)
        for name in files
        if "MNI" not in name and "necrosis" in name and name.endswith(".nii.gz")
    ]
    necrosis_paths.sort()
    edema_paths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(predicted_path)
        for name in files
        if "MNI" not in name and "edema" in name and name.endswith(".nii.gz")
    ]
    edema_paths.sort()
    enhancing_tumor_paths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(predicted_path)
        for name in files
        if "MNI" not in name and "enhancing" in name and name.endswith(".nii.gz")
    ]
