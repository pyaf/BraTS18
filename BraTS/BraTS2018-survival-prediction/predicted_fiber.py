...
def MoveLesionsMNIMask(brats_root, dst_dir, subject_names):
    ...
    for subject_name in subject_names:
        whole_tumor_mni_path = [
            ...
            and "whole_tumor" in name
            and "MNI152_1mm" in name
...
assert (
    len(survival_names) == 59 or len(survival_names) == 28 or len(survival_names) == 77
)
...
), "HCP1021 template is not in the dsi studio directory"
...
for idx, whole_tumor_file in enumerate(whole_tumor_files_dir):
    ...
    network_measure_file_dst = os.path.join(
        ...
        "network_measures",
        mode,
        os.path.split(network_measure_files[0].replace(source, pat_name))[1],
    )
    ...
    connectogram_files = [
        ...
        for root, dirs, files in os.walk(work_dir)
        for name in files
        if "connectogram" in name and name.endswith(".txt")
    ]
    connectogram_file_dst = os.path.join(
        os.path.split(connectogram_files[0])[0],
        "connectogram",
        mode,
        os.path.split(connectogram_files[0].replace(source, pat_name))[1],
    )
    ...
    connectivity_files = [
        ...
        for root, dirs, files in os.walk(work_dir)
        for name in files
        if "connectivity" in name and name.endswith(".mat")
    ]
    connectivity_file_dst = os.path.join(
        os.path.split(connectivity_files[0])[0],
        "connectivity",
        mode,
        os.path.split(connectivity_files[0].replace(source, pat_name))[1],
    )
