...
def binarize_connectivity_matrix(connectivity_matrix, threshold=0.01):
    ...
    binary_connectivity_matrix[
        connectivity_matrix >= threshold * np.amax(connectivity_matrix)
    ] = 1
...
def threshold_connectivity_matrix(connectivity_matrix, threshold=0.01):
    ...
    thresholded_connectivity_matrix[
        connectivity_matrix <= threshold * np.amax(connectivity_matrix)
    ] = 0
...
def get_weighted_connectivity_feature_vectors_test(
    dsi_studio_path=paths.dsi_studio_path
):
    connectivity_testing_dir = os.path.join(dsi_studio_path, "connectivity", "testing")
    whole_tumor_mni_testing_dir = os.path.join(
        dsi_studio_path, "predicted_whole_tumor", "testing"
    )

    connectivity_pass_files = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(connectivity_testing_dir)
        for name in files
        if "count" in name
        and "ncount" not in name
        and "connectivity" in name
        and "pass" in name
        and name.endswith(".mat")
    ]
    connectivity_pass_files.sort()

    connectivity_end_files = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(connectivity_testing_dir)
        for name in files
        if "count" in name
    ...
    whole_tumor_mni_paths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(whole_tumor_mni_testing_dir)
        for name in files
        if "whole_tumor" in name and "MNI152_1mm" in name and name.endswith("nii.gz")
    ]
    whole_tumor_mni_paths.sort()

    assert (
        len(connectivity_pass_files)
        == len(connectivity_end_files)
        == len(whole_tumor_mni_paths)
        == 77
    )

    W_dsi_pass_histogram_features = np.zeros(
        (len(connectivity_pass_files), 116), dtype=np.float32
    )
    W_nrm_pass_histogram_features = np.zeros(
        (len(connectivity_pass_files), 116), dtype=np.float32
    )
    W_bin_pass_histogram_features = np.zeros(
        (len(connectivity_pass_files), 116), dtype=np.float32
    )

    W_dsi_end_histogram_features = np.zeros(
        (len(connectivity_pass_files), 116), dtype=np.float32
    )
    W_nrm_end_histogram_features = np.zeros(
        (len(connectivity_pass_files), 116), dtype=np.float32
    )
    W_bin_end_histogram_features = np.zeros(
        (len(connectivity_pass_files), 116), dtype=np.float32
    )
    pat_names = []
    for (
        idx,
        (connectivity_pass_file, connectivity_end_file, whole_tumor_mni_path),
    ) in enumerate(
        zip(connectivity_pass_files, connectivity_end_files, whole_tumor_mni_paths)
    ):

        assert get_pat_name(connectivity_pass_file) == get_pat_name(
        ...
        weighted_connectivity_matrix_pass_temp = connectivity_matrix_pass_obj[
            "connectivity"
        ]
        weighted_connectivity_matrix_pass = threshold_connectivity_matrix(
            weighted_connectivity_matrix_pass_temp, 0
        )
        ...
        weighted_connectivity_matrix_end_temp = connectivity_matrix_end_obj[
            "connectivity"
        ]
        weighted_connectivity_matrix_end = threshold_connectivity_matrix(
            weighted_connectivity_matrix_end_temp, 0
        )
        W_nrm_end, W_bin_end = weight_conversion(weighted_connectivity_matrix_end)

        # weighted connectivity histogram
        W_dsi_pass_histogram_features[idx, :] = np.multiply(
            np.sum(weighted_connectivity_matrix_pass, axis=0), lesion_weights
        )
        W_nrm_pass_histogram_features[idx, :] = np.multiply(
            np.sum(W_nrm_pass, axis=0), lesion_weights
        )
        W_bin_pass_histogram_features[idx, :] = np.multiply(
            np.sum(W_bin_pass, axis=0), lesion_weights
        )

        W_dsi_end_histogram_features[idx, :] = np.multiply(
            np.sum(weighted_connectivity_matrix_end, axis=0), lesion_weights
        )
        W_nrm_end_histogram_features[idx, :] = np.multiply(
            np.sum(W_nrm_end, axis=0), lesion_weights
        )
        W_bin_end_histogram_features[idx, :] = np.multiply(
            np.sum(W_bin_end, axis=0), lesion_weights
        )
    return (
        pat_names,
        W_dsi_pass_histogram_features,
        W_nrm_pass_histogram_features,
        W_bin_pass_histogram_features,
        W_dsi_end_histogram_features,
        W_nrm_end_histogram_features,
        W_bin_end_histogram_features,
    )


def get_weighted_connectivity_feature_vectors_valid(
    dsi_studio_path=paths.dsi_studio_path
):
    connectivity_valid_dir = os.path.join(dsi_studio_path, "connectivity", "validation")
    whole_tumor_mni_valid_dir = os.path.join(
        dsi_studio_path, "predicted_whole_tumor", "validation"
    )

    connectivity_pass_files = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(connectivity_valid_dir)
        for name in files
        if "count" in name
        and "ncount" not in name
        and "connectivity" in name
    ...
    whole_tumor_mni_paths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(whole_tumor_mni_valid_dir)
        for name in files
        if "whole_tumor" in name and "MNI152_1mm" in name and name.endswith("nii.gz")
    ]
    whole_tumor_mni_paths.sort()
...
def get_weighted_connectivity_feature_vectors_train(
    dsi_studio_path=paths.dsi_studio_path, mode="gt"
):
    ...
    if mode == "predicted":
        connectivity_train_dir = os.path.join(
            dsi_studio_path, "connectivity", "training"
        )
        whole_tumor_mni_train_dir = os.path.join(
            dsi_studio_path, "predicted_whole_tumor", "training"
        )

    connectivity_pass_files = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(connectivity_train_dir)
        for name in files
        if "count" in name
        and "ncount" not in name
        and "connectivity" in name
    ...
    whole_tumor_mni_paths = [
        os.path.join(root, name)
        for root, dirs, files in os.walk(whole_tumor_mni_train_dir)
        for name in files
        if "whole_tumor" in name and "MNI152_1mm" in name and name.endswith("nii.gz")
    ]
    whole_tumor_mni_paths.sort()
...
parser.add_argument(
    "-m", "--mode", help="can be gt or predicted", default="gt", type=str
)
...
pat_names_train, gt, W_dsi_pass_histogram_features, W_nrm_pass_histogram_features, W_bin_pass_histogram_features, W_dsi_end_histogram_features, W_nrm_end_histogram_features, W_bin_end_histogram_features = get_weighted_connectivity_feature_vectors_train(
    mode=mode
)
...
pat_names_valid, W_dsi_pass_histogram_features_valid, W_nrm_pass_histogram_features_valid, W_bin_pass_histogram_features_valid, W_dsi_end_histogram_features_valid, W_nrm_end_histogram_features_valid, W_bin_end_histogram_features_valid = (
    get_weighted_connectivity_feature_vectors_valid()
)

# Testing
logging.info("loading testing set...")
pat_names_test, W_dsi_pass_histogram_features_test, W_nrm_pass_histogram_features_test, W_bin_pass_histogram_features_test, W_dsi_end_histogram_features_test, W_nrm_end_histogram_features_test, W_bin_end_histogram_features_test = (
    get_weighted_connectivity_feature_vectors_test()
)
...
normalized_W_bin_pass_histogram_features = scaler.fit_transform(
    W_bin_pass_histogram_features
)
...
normalized_W_bin_pass_histogram_features_valid = scaler.fit_transform(
    W_bin_pass_histogram_features_valid
)
...
normalized_W_bin_pass_histogram_features_test = scaler.fit_transform(
    W_bin_pass_histogram_features_test
)
...
selected_normalized_W_bin_pass_histogram_features = sel.transform(
    normalized_W_bin_pass_histogram_features
)
selected_normalized_W_bin_pass_histogram_features_valid = sel.transform(
    normalized_W_bin_pass_histogram_features_valid
)
selected_normalized_W_bin_pass_histogram_features_test = sel.transform(
    normalized_W_bin_pass_histogram_features_test
)
...
rskf = RepeatedStratifiedKFold(
    n_splits=n_splits, n_repeats=n_repeats, random_state=36851234
)
...
logging.info(
    "Best Scores of Weighted connectivity matrix  - Using SVM - Accuracy: %0.4f (+/- %0.4f)"
    % (svm_accuracy, svm_std)
)
...
df_valid.to_csv(
    "survival_W_bin_pass_linearscv_prob_valid.csv", header=False, index=False
)
