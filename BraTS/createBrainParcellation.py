...
def RegisterBrain(t1_path, ref_path, subject2mni_mat, mni2subject_mat):
    ...
    subprocess.call(
        ["flirt", "-in", t1_path, "-ref", ref_path, "-omat", subject2mni_mat]
    )
    subprocess.call(
        ["convert_xfm", "-omat", mni2subject_mat, "-inverse", subject2mni_mat]
    )
...
def SubjectLabels2ParcellationArgmax(subject_bp_filepaths, subject_name):
    ...
    subjectBrainParcellations = np.zeros(
        (len(subject_bp_filepaths) + 1, 155, 240, 240), dtype=np.float32
    )
...
parser.add_argument(
    "-i", "--input", help="the input directory to a BraTS subject T1 image", type=str
)
parser.add_argument(
    "-o",
    "--output",
    help="the output directory to save the subject's brain parcellation",
    type=str,
)
...
subject2mni_path = os.path.join(
    temp_dir, file_name[: file_name.index(".nii.gz")] + "_invol2refvol.mat"
)
mni2subject_path = os.path.join(
    temp_dir, file_name[: file_name.index(".nii.gz")] + "_refvol2invol.mat"
)
...
bp_filepaths = [
    ...
    for root, dirs, files in os.walk(brain_parcellation_path)
    for name in files
...
subject_bp_filepaths = [
    os.path.join(root, name)
    for root, dirs, files in os.walk(temp_dir)
    for name in files
    if "HarvardOxford" not in name and "lab" in name and name.endswith(".nii.gz")
]
