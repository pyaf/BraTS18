...
def main():
    ...
    with torch.no_grad():
        scores = validate(
            valid_loader, model, args.batch_size, args.out_dir, valid_set.names
        )
...
def test(valid_loader, model, batch_size, out_dir="", names=None, verbose=True):
...
def validate(
    valid_loader, model, batch_size, out_dir="", names=None, scoring=True, verbose=True
):
