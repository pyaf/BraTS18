...
for dim, shape in enumerate(patch_shape):
    dist2center[dim] = (
        [shape / 2 - 1, shape / 2] if shape % 2 == 0 else [shape // 2, shape // 2]
    )
...
def process(path):
    label = (
        1 if not has_label else nib_load(path + "seg.nii.gz").astype("int16")
    )  # some are uint8
    ...
    images = np.stack([nib_load(path + modal + ".nii.gz") for modal in modalities], -1)
