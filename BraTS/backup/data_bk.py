...
class ImageList(Dataset):
    def __init__(
        ...
        self.sub_patch_shape = get_sub_patch_shape(
            self.patch_shape, self.receptive_field, self.scale_factor
        )
    ...
    def coord_to_sub_slice(self, coord):
        ...
        hi = (
            lo
            + self.scale_factor * self.receptive_field
    ...
    def crop(self, coords, images, label):
        ...
        sub_samples = np.zeros(
            (N, self.C) + tuple(self.sub_patch_shape), dtype="float32"
        )
        ...
        for n, coord in enumerate(coords):
            ...
            pimages = (
                np.pad(images, [(0, 0)] + pad, mode="constant")
                if np.sum(pad) > 0
                else images
            )

            sub_samples[n] = pimages[
                :, lo[0] : hi[0] : kx, lo[1] : hi[1] : ky, lo[2] : hi[2] : kz
            ]
    ...
    def __getitem__(self, index):
        ...
        start = time.time()
        images = np.array(
            [nib_load(path + modal + ".nii.gz") for modal in self.modalities]
        )
        t1 = time.time() - start
        start = time.time()
...
class PEDataLoader(object):
    ...
    def __init__(
        ...
        self.pin_memory_fn = (
            torch.utils.data.dataloader.pin_memory_batch if pin_memory else lambda x: x
        )

        self.num_samples = len(dataset)
        self.num_batches = num_batches or int(
