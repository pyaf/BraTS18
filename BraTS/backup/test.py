...
def validate(valid_loader, model, criterion=None, out_dir="", names=None, scoring=True):
    h, w, t = 27, 27, 18
    H, W, T = 240, 240, 155
    dtype = torch.uint8
    ...
    for i, (data, labels) in enumerate(valid_loader):
        ...
        for b, (x1, x2, target) in enumerate(
            zip(*[d.split(args.batch_size) for d in data])
        ):
