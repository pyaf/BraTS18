...
class DualData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        samples = multicrop.crop3d(
            images, coords, sample_size, sample_size, sample_size, 1, False
        ).float()

        sub_samples = multicrop.crop3d(
            images, coords, sub_sample_size, sub_sample_size, sub_sample_size, 3, False
        ).float()

        target = (
            coords
            if not self.return_target
            else multicrop.crop3d(
                label, coords, target_size, target_size, target_size, 1, False
            ).long()
        )
